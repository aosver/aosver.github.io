...
if (isset($_GET['idEliminar'])){
  ...
  $sql = "SELECT * FROM tlibroautor WHERE idAutor = $id";
  $query = mysqli_query($conn, $sql)or die(mysqli_error($conn));
  if (mysqli_num_rows($query) == 0){
    $sql = "UPDATE tautores SET estado = 0 WHERE id = $id";
    $query = mysqli_query($conn, $sql)or die(mysqli_error($conn));  
  }else{
    $bError = 'No se puede eliminar el autor porque tiene libros relacionados a él.';
  }
}

if (isset($_POST['nueva'])){
  $nombre = utf8_decode($_POST['nombre']);
  $sql = "INSERT INTO tautores (id, nombre, estado) VALUES (null, '$nombre',1)";
  $query = mysqli_query($conn, $sql)or die(mysqli_error($conn));
}
...
<!--titulo de arriba-->
<title>Autores - Programación Alternativa</title>
<!--titulo de arriba-->
<style type="text/css">
body {
	padding: 0px;
...
<table width="100%" border="0" cellpadding="0" cellspacing="0">
  ...
  <?php if (isset($_GET['idModificar'])){ 
  	$id = $_GET['idModificar'];
  	$sql = "SELECT * FROM tautores WHERE id = $id";
  	$query = mysqli_query($conn, $sql);
  	...
  	?>
 <center>
    Modificar un autor<hr>
    <form action="autores.php?<?PHP echo $busquedas?>" method="post">
    ...
    <a href="autores.php?<?PHP echo $busquedas?>"><input type="button" value="Cancelar" class="button"></a>
    </form>
  </center>
  <?php }else{ ?>
  <center>
    Agregar un nuevo autor<hr>
  ...
  <?php 
    $sql = "SELECT * FROM tautores WHERE estado = 1";
    $query = mysqli_query($conn, $sql);
    ...
    $numero_registros = mysqli_num_rows($result);  
            while ($row=mysqli_fetch_assoc($result)) {
              ?>
                <tr>
                  <td>
                  <a href="autores.php?idModificar=<?php echo $row['id'] ?><?PHP echo $busquedas?>">
                  	<img src="img/edit.png" height="20">
                  </a> 
                  	<a href="autores.php?idEliminar=<?php echo $row['id'] ?><?PHP echo $busquedas?>" onclick="return confirm('Está seguro(a) que desea eliminar?');">
